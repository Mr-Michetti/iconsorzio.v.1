            ...
            <div className="mt-10 sm:hidden capitalize">
                ...
                <ul role="list" className="mt-3 border-t border-gray-200 divide-y divide-gray-100 ">
                    {tableData ? tableData.map((item) => (
                        <li key={item.id}>
                            <Link href={`/admin/patenti/edit/${item.id}`} className="group flex items-center justify-between px-4 py-4 hover:bg-gray-50 sm:px-6">
                            ...
                            {tableData ? tableData.map((item) => (
                                <tr key={item.id} className={selectedRow === item.id ? notModified ? 'bg-yellow-100' : 'bg-green-100' : ''}>
                                    ...
                                    <td className="px-6 py-3 max-w-0 w-full whitespace-nowrap text-sm font-medium text-gray-900">
                                        ...
                                        <div className="flex items-center space-x-3 lg:pl-2">
                                            <div
                                                ...
                                                aria-hidden="true"
                                            />
                                            <Link href={`/admin/patenti/edit/${item.id}`} className="truncate hover:text-gray-600">
                                                {item.nome}
                                            </Link>
                                        </div>
                                    ...
                                    <td className="px-6 py-3 whitespace-nowrap text-right text-sm font-medium">
                                        <Link href={`/admin/patenti/edit/${item.id}`} className="text-indigo-600 hover:text-indigo-900">
                                            <ChevronRightIcon
