            ...
            <div className="mt-10 sm:hidden capitalize">
                ...
                <ul role="list" className="mt-3 border-t border-gray-200 divide-y divide-gray-100 ">
                    {(tableData && tableData.length > 0) ? tableData.map((item) => (
                        <li key={item.id}>
                            <Link href={`/admin/workplace/edit/${item.id}`} className="group flex items-center justify-between px-4 py-4 hover:bg-gray-50 sm:px-6">
                        ...
                        <thead>
                            <tr className="border-t border-gray-200">
                                <th className="px-6 py-3 border-b border-gray-200 bg-gray-50 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                    <span className="lg:pl-2">Nome patente</span>
                                </th>
                            ...
                            {(tableData && tableData.length > 0) ? tableData.map((item) => (
                                <tr key={item.id} className={selectedRow === item.id ? notModified ? 'bg-yellow-100' : 'bg-green-100' : ''}>
                                    ...
                                    <td className="px-6 py-3 max-w-0 w-full whitespace-nowrap text-sm font-medium text-gray-900">
                                        ...
                                        <div className="flex items-center space-x-3 lg:pl-2">
                                            <div
                                                ...
                                                aria-hidden="true"
                                            />
                                            <Link href={`/admin/workplace/edit/${item.id}`} className="truncate hover:text-gray-600">
                                                {item.nome}
                                            </Link>
                                        </div>
                                    ...
                                    <td className="px-6 py-3 whitespace-nowrap text-right text-sm font-medium">
                                        <Link href={`/admin/workplace/edit/${item.id}`} className="text-indigo-600 hover:text-indigo-900">
                                            <ChevronRightIcon
