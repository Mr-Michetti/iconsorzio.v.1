...
export default async function handler(req, res) {
    ...
    try {
        ...
        const veicoliAttivi = await prisma.veicolo.count({
            where: {
                companyId: getActiveCompany.isActive,
                isActive: true
            }
        });
        const veicoliDisattivati = await prisma.veicolo.count({
            where: {
                companyId: getActiveCompany.isActive,
                isActive: false
            }
        ...
        res.status(200).json({ active: veicoliAttivi, deactivated: veicoliDisattivati })
    }
    catch (err) {
        console.log(err)
